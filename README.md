...
cd powermill-ai
setup.bat                  :: venv + зависимости + папки на E: + проверка
start_download_models.bat  :: модели Ollama (~5 ГБ)
```
...
```bat
1) start_menu.bat  ->  пункт 14  (скачать модели Ollama)
2) start_menu.bat  ->  пункт 9   (посмотреть, что в папке справки)
3) start_menu.bat  ->  пункт 5   (пробный разбор 30 страниц + отчёт)
4) start_menu.bat  ->  пункт 4   (полный разбор справки)
5) start_menu.bat  ->  пункт 6   (векторная база; можно на ночь — пункт 7)
6) start_menu.bat  ->  пункт 1   (общаться)
```
